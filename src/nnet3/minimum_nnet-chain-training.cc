...
#include "nnet3/lyg-nnet-chain-training.h"
...
NnetChainTrainer::NnetChainTrainer(const NnetChainTrainingOptions &opts,
                                   const fst::StdVectorFst &den_fst, Nnet *nnet)
    : opts_(opts), den_graph_(den_fst, nnet->OutputDim("output")), nnet_(nnet),
      compiler_(*nnet, opts_.nnet_config.optimize_config,
                opts_.nnet_config.compiler_config),
      num_minibatches_processed_(0), srand_seed_(RandInt(0, 100000)) {
  // std::cout << "opts.nnet_config.zero_component_stats: " << opts.nnet_config.zero_component_stats << std::endl;
  ...
  // std::cout << "opts.nnet_config.read_cache: " << opts.nnet_config.read_cache << std::endl;
...
void NnetChainTrainer::Train(const NnetChainExample &chain_eg) {
  ...
  //     num_minibatches_processed_ % nnet_config.backstitch_training_interval ==
  ...
  // } else { // conventional training
    // std::cout << "TrainInternal function called" << std::endl;
...
void NnetChainTrainer::TrainInternal(const NnetChainExample &eg,
  ...
  // store stats.
  NnetComputer computer(nnet_config.compute_config, computation, nnet_,
                        delta_nnet_);
  ...
  // // regularization.
  // ApplyL2Regularization(*nnet_, GetNumNvalues(eg.inputs, false) *
  //                                   nnet_config.l2_regularize_factor,
  //                       delta_nnet_);

  // Updates the parameters of nnet
  bool success = UpdateNnetWithMaxChange(
      *delta_nnet_, nnet_config.max_param_change, 1.0,
      1.0 - nnet_config.momentum, nnet_, &num_max_change_per_component_applied_,
  ...
  // Scale delta_nnet
  // std::cout << "success for scale delta_nnet" << success << std::endl;
  if (success)
...
//     const NnetChainExample &eg, const NnetComputation &computation,
...
//
//  // Updates the parameters of nnet
//  UpdateNnetWithMaxChange(*delta_nnet_, nnet_config.max_param_change,
...
void NnetChainTrainer::ProcessOutputs(bool is_backstitch_step2,
  ...
  // std::cout << "modified processoutputs" << std::endl;
  ...
  for (; iter != end; ++iter) {
    ...
    if (node_index < 0 || !nnet_->IsOutputNode(node_index))
      KALDI_ERR << "Network has no output named " << sup.name;

    const CuMatrixBase<BaseFloat> &nnet_output = computer->GetOutput(sup.name);
    // std::cout << "nnet_output" << nnet_output << std::endl;
    CuMatrix<BaseFloat> nnet_output_deriv(nnet_output.NumRows(),
                                          nnet_output.NumCols(), kUndefined);
    ...
    ComputeChainObjfAndDeriv(opts_.chain_config, den_graph_, sup.supervision,
                             nnet_output, &tot_objf, &tot_l2_term, &tot_weight,
                             &nnet_output_deriv,
                             NULL);
    ...
    //       computer->GetOutput(xent_name);
    //   // at this point, xent_deriv is posteriors derived from the numerator
    //   // computation.  note, xent_objf has a factor of '.supervision.weight'
    //   BaseFloat xent_objf = TraceMatMat(xent_output, xent_deriv, kTrans);
    //   objf_info_[xent_name + suffix].UpdateStats(
    //       xent_name + suffix, opts_.nnet_config.print_interval,
    //       num_minibatches_processed_, tot_weight, xent_objf);
    // }
    ...
    objf_info_[sup.name + suffix].UpdateStats(
        sup.name + suffix, opts_.nnet_config.print_interval,
        num_minibatches_processed_, tot_weight, tot_objf, tot_l2_term);
...
//   unordered_map<std::string, ObjectiveFunctionInfo,
//                 StringHasher>::const_iterator iter = objf_info_.begin(),
//                                               end = objf_info_.end();
//   bool ans = false;
//   for (; iter != end; ++iter) {
...
//         KALDI_LOG
//             << "For " << delta_nnet_->GetComponentName(c)
//             << ", per-component max-change was enforced "
//             << (100.0 * num_max_change_per_component_applied_[i]) /
//                    (num_minibatches_processed_ *
//                     (nnet_config.backstitch_training_scale == 0.0
//                          ? 1.0
//                          : 1.0 +
//                                1.0 / nnet_config.backstitch_training_interval))
//             << " \% of the time.";
//       i++;
//     }
//   }
...
//     KALDI_LOG
//         << "The global max-change was enforced "
